#!/bin/bash
#
# Coding test question number 4.  Steps:
#
...
# NOTES:
#   - The remote Github repo is here: https://github.com/buzzbombnc/ict_q4code.git
#   - The actual service is a mocked up HTTP server using Python.
#   - The execution server is CentOS 7 and the destination server is Ubuntu 16.10.
#   - Both servers have a 'testuser' account and the SSH keys are setup from exection --> destination.
#   - Both also have the prereqs installed: curl, python (2.7), git.
...
# Configuration
REPO=https://github.com/buzzbombnc/ict_q4code.git
